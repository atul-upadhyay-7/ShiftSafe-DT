...
import type { WorkerProfile, PolicyData, ClaimData } from '@/backend/utils/store';
import { getSeededClaims, generatePayoutRef, getTriggerEmoji, getTriggerName } from '@/backend/utils/store';
...
interface AppContextType extends AppState {
  ...
  recalculatePremium: (zone: string, earnings: number, platform: string) => PolicyData;
}
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const addClaim = useCallback((c: ClaimData) => {
    setState(prev => ({
      ...
      totalEarningsProtected: prev.totalEarningsProtected + c.amount,
    }));
  }, []);

  const login = useCallback((worker: WorkerProfile, policy: PolicyData) => {
    const seededClaims = getSeededClaims(worker.avgWeeklyEarnings);
    const totalProtected = seededClaims.reduce((s, c) => s + c.amount, 0);
    setState({
      worker,
      policy,
      claims: seededClaims,
  ...
  const simulateTrigger = useCallback((triggerType: string): ClaimData => {
    const earnings = state.worker?.avgWeeklyEarnings || 4200;
    const dailyEarnings = earnings / 7;
    const pct = COVERAGE_PCT[triggerType] ?? 0.50;
  ...
  const recalculatePremium = useCallback((zone: string, earnings: number, platform: string): PolicyData => {
    const premium = calculateWeeklyPremium(zone, earnings, platform, state.claims.length);
    const newPolicy: PolicyData = {
      ...
      status: 'active',
      startDate: state.policy?.startDate || new Date().toISOString().split('T')[0],
      nextPaymentDue: state.policy?.nextPaymentDue || new Date(Date.now() + 7 * 86400000).toISOString().split('T')[0],
