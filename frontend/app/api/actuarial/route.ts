...
export async function GET() {
  ...
  const snapshot = getActuarialSnapshot(
    totalPremium, totalClaims, activePolicies, totalWorkers, 'Current Period'
  );
...
export async function POST(req: NextRequest) {
  try {
    ...
    let result;
    if (scenario === 'monsoon') {
      result = runMonsoonStressScenario(workers, avgPremium, avgDailyPayout || 450);
    } else if (scenario === 'delhi_hazard') {
      result = runDelhiHazardScenario(workers, avgPremium, avgDailyPayout || 300);
    } else {
    ...
    await db.prepare(`INSERT INTO stress_scenarios (id, scenario_name, scenario_type, duration_days, trigger_frequency, avg_payout_per_day, total_estimated_payout, total_premium_in_period, bcr_under_stress, loss_ratio_under_stress, is_sustainable, recommendation)
      ...
      result.totalEstimatedPayout, result.totalPremiumInPeriod,
      result.bcrUnderStress, result.lossRatioUnderStress,
      result.isSustainable ? 1 : 0, result.recommendation,
    );
